...
class RoomProgressStorage(private val progressDao: ProgressDao): LocalProgressStorage {
    ...
    private fun convertToRoomProgressModel(movie: ProgressModel) = RoomProgressModel(
        movie.id,
        movie.progress
    )

    private fun convertToProgressModel(movie: RoomProgressModel) = ProgressModel(
        movie.id,
        movie.progress
    )
}